...
/* track number of load */
static counter_t contor_loads = 0;

/* track number of store */
...
"  Predictor `comb' combines a bimodal and a 2-level predictor.\n"
  ...
  opt_reg_uint(odb, "-contor:LD", "number of Load and Store instructions",
	       &opt_load, /* default */0,
	       /* print */TRUE, /* format */NULL);

    /* First n prime numbers */
  opt_reg_uint(odb, "-history", "first n prime numbers",
	       &opt_prime, /* default */0,
	       /* print */TRUE, /* format */NULL);
...
#include "machine.def"
	...
	dlite_main(regs.regs_PC, regs.regs_NPC, sim_num_insn, &regs, mem);
      ...
      void NrPrime(int opt_prime) 
      {
  	 int i, j, flag, foundedNumbers;
   	 printf("First %d prime numbers are: \n", opt_prime);
   	 for (i = 2, foundedNumbers = 0; foundedNumbers < opt_prime; i++) 
   	{
        flag = 0;
         for (j = 2; j * j <= i; j++)
        {
